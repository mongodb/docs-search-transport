...
async function getManifestsFromS3(bucketName, prefix) {
    ...
    const result = await util_1.default.promisify(s3.makeUnauthenticatedRequest.bind(s3, 'listObjectsV2', {
        Bucket: bucketName,
    ...
    for (const bucketEntry of result.Contents || []) {
        ...
        const data = await util_1.default.promisify(s3.makeUnauthenticatedRequest.bind(s3, 'getObject', {
            Bucket: bucketName,
            Key: bucketEntry.Key,
        }))();
...
function getManifestsFromDirectory(prefix) {
    return new Promise((resolve, reject) => {
        const manifests = [];
        dive_1.default(prefix, async (err, path, stats) => {
            if (err) {
                reject(err);
            }
